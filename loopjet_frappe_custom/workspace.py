...
CUSTOMER_REPORTS_SHORTCUT_LABEL = "Kundenberichte"
CUSTOMER_REPORTS_SHORTCUT_URL = "/app/loopjet-customer-report"
CUSTOMER_REPORTS_SHORTCUT_BLOCK_ID = "loopjet-customer-reports"
CUSTOMER_REPORTS_SIDEBAR_ICON = "file-text"
...
def customer_reports_sidebar_item_values() -> dict[str, Any]:
	return {
		"label": CUSTOMER_REPORTS_SHORTCUT_LABEL,
		"type": "Link",
		"link_type": "URL",
		"link_to": "",
		"url": CUSTOMER_REPORTS_SHORTCUT_URL,
		"icon": CUSTOMER_REPORTS_SIDEBAR_ICON,
	}


def reconcile_customer_reports_sidebar_item(item: Any) -> bool:
	"""Repair an existing customer-reports sidebar row and report whether it changed."""
	changed = False
	for fieldname, value in customer_reports_sidebar_item_values().items():
		if item.get(fieldname) == value:
			continue
		if hasattr(item, "set"):
			item.set(fieldname, value)
...
def add_customer_reports_shortcut_to_layout(content: str) -> tuple[str, bool]:
	"""Add the customer-reports shortcut block to a Workspace layout once."""
	layout: list[dict[str, Any]] = json.loads(content or "[]")
	if any(
		block.get("type") == "shortcut"
		and block.get("data", {}).get("shortcut_name") == CUSTOMER_REPORTS_SHORTCUT_LABEL
		for block in layout
	):
		return content, False
	...
	insert_at = next(
		(
			...
			and item.get("data", {}).get("shortcut_name") == RAVEN_SHORTCUT_LABEL
		),
		next(
			(
				index + 1
...
def install_customer_reports_home_shortcut() -> bool:
	"""Expose the internal customer-report register in the ERPNext Home workspace."""
	import frappe
	...
	if frappe.db.exists("Workspace", "Home"):
		workspace = frappe.get_doc("Workspace", "Home")
		shortcut = next(
			(row for row in workspace.shortcuts if row.label == CUSTOMER_REPORTS_SHORTCUT_LABEL),
			None,
		)
		values = {
			"type": "URL",
		...
		workspace.content, layout_changed = add_customer_reports_shortcut_to_layout(workspace.content)
		workspace_changed = workspace_changed or layout_changed
		if workspace_changed:
			workspace.flags.ignore_permissions = True
			workspace.save()
	...
	changed = install_customer_reports_home_sidebar_link() or changed
	if changed:
		frappe.clear_cache()
	return changed


def install_customer_reports_home_sidebar_link() -> bool:
	"""Add customer reports to Frappe v16's dedicated Home sidebar, including user copies."""
	import frappe
	...
	for sidebar_name in dict.fromkeys(sidebar_names):
		...
		item = next(
			(
				row
				for row in sidebar.items
				if row.label == CUSTOMER_REPORTS_SHORTCUT_LABEL
				or (row.link_type == "URL" and row.url == CUSTOMER_REPORTS_SHORTCUT_URL)
			),
			None,
		)
		sidebar_changed = False
		if item is None:
			sidebar.append("items", customer_reports_sidebar_item_values())
			sidebar_changed = True
		else:
			sidebar_changed = reconcile_customer_reports_sidebar_item(item)
